...
# Using a wildcard (*) ensures it doesn't crash if package-lock.json isn't generated yet!
COPY app/package*.json ./
RUN npm install --omit=dev

# TODO(step-4c): copy the rest of the app source into /app.
# This copies everything inside the local /app folder into the container's /app folder.
COPY app/ ./
